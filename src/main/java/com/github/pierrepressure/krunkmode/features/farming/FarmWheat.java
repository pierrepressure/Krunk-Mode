...
public class FarmWheat extends FarmCrop {

    public static final FarmWheat INSTANCE = new FarmWheat();
        ...
        if (now - lastStepTime >= STEP_DURATIONS[currentStep]) {
            ...
            if (currentStep >= STEP_DURATIONS.length) {
                ...
                if (autoPauseLoops != 0 && (autoPLoopsCounter >= autoPauseLoops)) {
                    mc.thePlayer.addChatMessage(new ChatComponentText(String.format("§6[KM] Farming Wheat Completed §a§l%d §6Loops", autoPauseLoops)));
                    autoPaused = true;
                    pause();
                    autoPLoopsCounter = 0;
                }
