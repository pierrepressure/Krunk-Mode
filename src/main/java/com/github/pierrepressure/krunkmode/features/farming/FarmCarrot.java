...
public class FarmCarrot extends FarmCrop {

    public static final FarmCarrot INSTANCE = new FarmCarrot();
        ...
        if (now - lastStepTime >= STEP_DURATIONS[currentStep]) {
            ...
            if (currentStep >= STEP_DURATIONS.length) {
                ...
                if (autoPauseLoops != 0 && (autoPLoopsCounter >= autoPauseLoops)) {
                    mc.thePlayer.addChatMessage(new ChatComponentText(String.format("§6[KM] Farming Carrot Completed §a§l%d §6Loops", autoPauseLoops)));
                    autoPaused = true;
                    pause();
                    autoPLoopsCounter = 0;
                }
