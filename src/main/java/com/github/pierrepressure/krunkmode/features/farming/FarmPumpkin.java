...
public class FarmPumpkin extends FarmCrop {

    public static final FarmPumpkin INSTANCE = new FarmPumpkin();
            ...
            if (currentStep >= STEP_DURATIONS.length) {
                ...
                if (loopCount >= 4) {
                    ...
                    if (autoPauseLoops != 0 && (autoPLoopsCounter >= autoPauseLoops)) {
                        mc.thePlayer.addChatMessage(new ChatComponentText(String.format("§6[KM] Farming Pumpkin Completed §a§l%d §6Loops", autoPauseLoops)));
                        pause();
                        autoPLoopsCounter = 0;
                        autoPaused = true;
                    }
